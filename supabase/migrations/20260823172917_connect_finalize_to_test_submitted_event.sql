-- FB Language Center
-- Migration: connect_finalize_to_test_submitted_event
--
-- Purpose:
--   1. Guarantee idempotency for automatic lifecycle events.
--   2. Create TEST_SUBMITTED when finalize_test_attempt() successfully
--      transitions an attempt to under_review.
--
-- Important:
--   This migration only persists the communication event.
--   It does not send WhatsApp messages.

begin;

create unique index if not exists
  communication_events_attempt_lifecycle_unique_idx
...
declare
  ...
  v_estimated_level text := 'Pendente';
begin
  v_user_id := auth.uid();
  ...
  inner join
    public.question_options qo
      on qo.id = ta.selected_option_id
      and qo.question_id = ta.question_id
  where
    ta.attempt_id = p_attempt_id
    and ts.test_id = v_attempt.test_id
    and q.is_active = true
    and q.category::text in (
      'grammar',
      'listening'
  ...
  set
    awarded_points =
      case
        when qo.is_correct then q.points
        else 0
      end,
    updated_at = now()
  from
  ...
  where
    ...
    and ts.id = q.section_id
    and ts.test_id = v_attempt.test_id
    and qo.id = ta.selected_option_id
    and qo.question_id = ta.question_id
    and q.is_active = true
    and q.category::text in (
      'grammar',
  ...
  select
    coalesce(sum(ta.awarded_points), 0)
  into
    v_objective_score
  from
    public.test_answers ta
  ...
  v_percentage :=
    round(
      (v_objective_score / v_objective_max) * 100,
      2
    );
  ...
  set
    status = 'under_review'::public.attempt_status,
    submitted_at = now(),
    updated_at = now()
  where
    id = p_attempt_id;
